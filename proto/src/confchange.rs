...
/// Parses a Space-delimited sequence of operatins(空格分隔的操作序列) into a slice of ConfChangeSingle. The supported operations are:
/// - vn: make n a vote
/// - ln: make n a learner
/// - rn: remove n
pub fn parse_conf_change(s: &str) -> Result<Vec<ConfChangeSingle>, String>{
    ...
    for tok in splits{
        ...
        let mut cc: ConfChangeSingle = ConfChangeSingle::default();
        ...
        cc.node_id = match chars.as_str().parse(){
            Ok(id) => id,
            Err(e) => return Err(format!("parse tokne {} fial: {}", tok, e)),
        };
        ccs.push(cc);
    }
    Ok(ccs)
...
/// The inverse(反向) to `parse_config_change`
...
/// Abstracts(抽象) over ConfChangeV2 and (legacy) ConfChange to allow treating(处理) them in a unified manner(统一的方式).
pub trait ConfChangeI {
    /// Conevrt conf change to ConfChangeV2
    fn into_v2(self) -> ConfChangeV2;
    /// Gets conf change as ConfChangeV2
    fn as_v2(&self) -> Cow<ConfChangeV2>;
    /// Converts conf change to ConfChange. ConfChangeV2 can't be changed to ConfChange.
...
impl ConfChangeV2 {
    /// Checks if uses Joint Consensus(联合共识).
    ///
    /// It will return Some if and only if this config change will use Joint Consensus, which is the case if it contains more than one change or if the use of Joint
    /// Consensus was requested explicitly. The bool indicates whether the Joint State will be left automatically.
    pub fn enter_joint(&self) -> Option<bool> {
        // NB: in theory(在理论上), more config changes could qualify for the "simple" protocol but it depends on the config on top of which the changes apply.
        // For example, adding two learners is not OK if both nodes are part of the base config (i.e. two voters are turned into learners in the process of
        // applying the conf change). In practice, these distinctions should not matter, so we keep it simple and use Joint Consensus(联合共识) liberally.
