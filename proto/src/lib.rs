...
// We used `default` method a lot to be support prost and rust-protobuf at the same time. And reassignment can be optimized compiler.
...
mod protos{
    ...
    impl Snapshot{
        /// For a given snapshot, determine if it's empty or not(对于给定的快照，决定他是否为空).
...
pub mod prelude {
    pub use crate::eraftpb::{
        ConfChange, ConfChangeSingle, ConfChangeTransition, ConfChangeType, ConfChangeV2, ConfState, Entry, EntryType, HardState, Message, MessageType, 
...
pub mod util {
    ...
    {
        fn from((votes, learners): (Iter1, Iter2)) -> Self {
            let mut conf_state = ConfState::default();
            conf_state.mut_voters().extend(votes.into_iter());
