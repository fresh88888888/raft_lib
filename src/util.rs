...
use slog::{OwnedKVList, record_static, KV, Record, b};

use crate::HashSet;
use protobuf::Message as PbMessage;
use crate::eraftpb::{Message, Entry};
...
/// Get the mahority number of given nodes count
#[inline]
...
impl <'a> Union<'a> {
    ...
    /// Returns an iterator iterates the distinct value in two sets.
    ...
    /// Gets the count of the union. The time complexity is O(n).
...
pub(crate) fn format_kv_list(kv_list: &OwnedKVList) -> String {
    ...
    kv_list.serialize(&Record::new(&record, &format_args!(""), b!()), &mut formatter).unwrap();
    formatter.buffer
}

/// Get the approximate size of entry(获取入口的大致大小)
#[inline]
pub fn entry_approximate_size(e: &Entry) -> usize{
    ...
    // }
    // Each field has tag(1 byte) if it's not default value. Tips: x bytes can represent a value up to 1 << x*7 - 1, So 1 byte => 127, 2 bytes => 16383, 3 bytes => 2097151.
    // If entry_type is normal(default), in general, the size should be tag(4) + term(1) + index(2) + data(2) + context(1) = 10. If entry_type is conf change, in general, the size should be
    // tag(5) + entry_type(1) + term(1) + index(2) + data(1) + context(1) = 11. We choose 12 in case of large index or large data for normal entry.
...
pub fn is_continuous_ents(msg: &Message, ents: &[Entry]) -> bool {
    if !msg.entries.is_empty() && !ents.is_empty() {
        let exppected_next_idx = msg.entries.last().unwrap().index + 1;
        return exppected_next_idx == ents.first().unwrap().index;
    }
    true
}
